...
class WechatRuntime {
  constructor(config) {
    this.config = config;
    this.runtimeEnv = buildRuntimeEnv(process.env, { proxyUrl: config.proxyUrl });
  ...
  async dispatchTextCommand(normalized) {
    switch (normalized.command) {
      case "status":
        ...
        return true;
      case "account":
        await this.handleAccountCommand(normalized);
        return true;
      case "quota":
        await this.handleQuotaCommand(normalized);
        return true;
      case "bind":
  ...
  async handleStatusCommand(normalized) {
    ...
    const inactivityStatus = getInactivityStatus(this, threadId);
    if (inactivityStatus) {
      await this.sendReplyToNormalized(normalized, buildQueriedInactivityStatusText(this, inactivityStatus));
      return;
    }
  ...
  async handleAccountCommand(normalized) {
    const summary = await readCurrentCodexAccountSummary();
    await this.sendReplyToNormalized(normalized, buildAccountInfoText(summary));
  }

  async handleQuotaCommand(normalized) {
    await this.sendReplyToNormalized(normalized, buildQuotaInfoText(this.latestRateLimits));
  }
  ...
  async handleCodexMessage(message) {
    updateLatestRateLimits(this, message?.params || null);
