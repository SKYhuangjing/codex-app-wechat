...
class SessionStore {
  ...
  listWorkspaceRoots(bindingKey) {
    ...
    return Object.keys(threadIdByWorkspaceRoot).sort((left, right) => left.localeCompare(right));
  }
  ...
  setPendingNewThreadForWorkspace(bindingKey, workspaceRoot, pending = true) {
    const normalizedWorkspaceRoot = normalizeValue(workspaceRoot);
    if (!normalizedWorkspaceRoot) {
      return this.getBinding(bindingKey);
    }
    ...
    const pendingNewThreadByWorkspaceRoot = {
      ...(current.pendingNewThreadByWorkspaceRoot || {}),
      [normalizedWorkspaceRoot]: !!pending,
    };
  ...
  clearPendingNewThreadForWorkspace(bindingKey, workspaceRoot) {
    return this.setPendingNewThreadForWorkspace(bindingKey, workspaceRoot, false);
  }
  ...
  removeWorkspace(bindingKey, workspaceRoot) {
    ...
    const pendingNewThreadByWorkspaceRoot = { ...(current.pendingNewThreadByWorkspaceRoot || {}) };
